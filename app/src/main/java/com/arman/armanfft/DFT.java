...
public class DFT {
    public static Mat FFT(Mat g) {
        Mat padded = new Mat();                     //expand input image to optimal size
        int m = Core.getOptimalDFTSize(g.rows());
        int n = Core.getOptimalDFTSize(g.cols()); // on the border add zero values
        Core.copyMakeBorder(g, padded, 0, m - g.rows(), 0, n - g.cols(), Core.BORDER_CONSTANT, Scalar.all(0));
